...
class DirectVoxGO(torch.nn.Module):
    def __init__(self, xyz_min, xyz_max,
                 ...
                 viewbase_pe=10, activation='relu',
                 **kwargs):
        ...
        self.rgbnet_kwargs = {
            ...
            'viewbase_pe': viewbase_pe, 'activation': activation,
        }
        self.rgbnet_full_implicit = rgbnet_full_implicit

        # Create voxel grid with coordinate values
        ...
        # Get the frequency embeddings of the voxel grid
        self.register_buffer('viewfreq', torch.FloatTensor([(2**i) for i in range(viewbase_pe)]))
        self_grid_xyz = self_grid_xyz.cpu()
        self_grid_xyz_emb = (self_grid_xyz.unsqueeze(-1) * self.viewfreq).flatten(-2)
        self_grid_xyz_emb = torch.cat([self_grid_xyz, self_grid_xyz_emb.sin(), self_grid_xyz_emb.cos()], -1)
        self_grid_xyz_emb = self_grid_xyz_emb.reshape(1, *self.world_size, dim0).permute(0, 4, 1, 2, 3)
        self.register_buffer('k0', torch.FloatTensor(self_grid_xyz_emb))

        if activation == 'relu':
            print('Activation is relu')
            activation_unit = nn.ReLU
        elif activation == 'silu':
            print('Activation is silu')
            activation_unit = nn.SiLU

        self.rgbnet = nn.Sequential(
            nn.Linear(dim0, rgbnet_width), activation_unit(inplace=True),
            *[
                nn.Sequential(nn.Linear(rgbnet_width, rgbnet_width), activation_unit(inplace=True))
        ...
        print('dvgo: density mlp', self.densitynet)
        print('dvgo: rgb mlp', self.rgbnet)
        ...
        if mask_cache_path is not None and mask_cache_path:
            ...
            self_grid_xyz = torch.stack(torch.meshgrid(
                torch.linspace(self.xyz_min[0], self.xyz_max[0], self.k0.shape[2]),
                torch.linspace(self.xyz_min[1], self.xyz_max[1], self.k0.shape[3]),
                torch.linspace(self.xyz_min[2], self.xyz_max[2], self.k0.shape[4]),
            ), -1)
            mask = mask_cache(self_grid_xyz)
        else:
            mask = torch.ones(list(self.world_size), dtype=torch.bool)
        self.mask_cache = MaskCache(path=None, mask=mask, xyz_min=self.xyz_min, xyz_max=self.xyz_max)
    ...
    def scale_volume_grid(self, num_voxels):
        ...
        dim0 = (3+3*self.rgbnet_kwargs['viewbase_pe']*2)
        self_grid_xyz = torch.stack(torch.meshgrid(
            torch.linspace(self.xyz_min[0], self.xyz_max[0], self.world_size[0]),
            torch.linspace(self.xyz_min[1], self.xyz_max[1], self.world_size[1]),
            torch.linspace(self.xyz_min[2], self.xyz_max[2], self.world_size[2]),
        ...
        """
        Generate new fourier embeddings for the newly scaled coordinates
        """
        # self_grid_xyz = self_grid_xyz.cpu()
        self_grid_xyz_emb = (self_grid_xyz.unsqueeze(-1) * self.viewfreq).flatten(-2)
        self_grid_xyz_emb = torch.cat([self_grid_xyz, self_grid_xyz_emb.sin(), self_grid_xyz_emb.cos()], -1)
        self_grid_xyz_emb = self_grid_xyz_emb.reshape(1, *self.world_size, dim0).permute(0, 4, 1, 2, 3).to(self.k0.device)
        self.k0 = self_grid_xyz_emb

        """
        Directly interpolate the old fourier embeddings
        """
        # self.k0 = torch.nn.Parameter(F.interpolate(self.k0.data, size=tuple(self.world_size), mode='trilinear', align_corners=True))

        mask_cache = self.mask_cache
        density = self.densitynet(self.k0.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)
        self_alpha = F.max_pool3d(self.activate_density(density), kernel_size=3, padding=1, stride=1)[0,0]
    ...
    def density_total_variation(self):
        density = self.densitynet(self.k0.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)
        alpha = self.activate_density(density, 1)
        return total_variation(alpha)

    def k0_total_variation(self):
        rgb_logit = self.rgbnet(self.k0.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)
        rgb = torch.sigmoid(rgb_logit)
        return total_variation(rgb)
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=None, **render_kwargs):
        ...
        """
        Get density using MLP (masked)
        """
        comb_mask = self.mask_cache.mask

        k0_density = (torch.zeros((self.k0.shape[0], 1, self.k0.shape[2] * self.k0.shape[3] * self.k0.shape[4])) - 100)
        k0_rgb = torch.zeros((self.k0.shape[0], 3, self.k0.shape[2] * self.k0.shape[3] * self.k0.shape[4]))

        k0_mask = self.k0.reshape(self.k0.shape[0], self.k0.shape[1], -1)[:, :, comb_mask.reshape(-1)]
        k0_density_mask = self.densitynet(k0_mask.permute(0, 2, 1)).permute(0, 2, 1)
        k0_density[:, :, comb_mask.reshape(-1)] = k0_density_mask
        k0_density = k0_density.reshape(self.k0.shape[0], 1, self.k0.shape[2], self.k0.shape[3], self.k0.shape[4])
        density = self.grid_sampler(ray_pts, k0_density)
        ...
        Get density using MLP (masked)
        """
        k0_rgb_mask = self.rgbnet(k0_mask.permute(0, 2, 1)).permute(0, 2, 1)
        k0_rgb[:, :, comb_mask.reshape(-1)] = k0_rgb_mask
        k0_rgb = k0_rgb.reshape(self.k0.shape[0], 3, self.k0.shape[2], self.k0.shape[3], self.k0.shape[4])
        rgb_logit = self.grid_sampler(ray_pts, k0_rgb)
        rgb = torch.sigmoid(rgb_logit)
...
class MaskCache(nn.Module):
    def __init__(self, path=None, mask_cache_thres=None, mask=None, xyz_min=None, xyz_max=None):
        ...
        if path is not None:
            ...
            # Get old model (this is needed since density isn't implict)
            old_model = DirectVoxGO(**st['model_kwargs'])
            old_model.load_state_dict(st['model_state_dict'])
            device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
            old_model = old_model.to(device)
            with torch.no_grad():
                old_model_density = old_model.densitynet(old_model.k0.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)

            self.mask_cache_thres = mask_cache_thres
            density = F.max_pool3d(old_model_density, kernel_size=3, padding=1, stride=1)
